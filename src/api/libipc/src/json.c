...
int json_parse(char *json, size_t json_len, json_t *kv_pairs,
               size_t max_pairs) {
  ...
  // Parse JSON
  int tokens_parsed;
  if ((tokens_parsed = jsmn_parse(&parser, json, json_len, tokens, jsmn_toks)) <
      0) {
  ...
  for (int x = 0; x < tokens_parsed && nth_pair < max_pairs; x++) {
    ...
    if (tokens[x].type == JSMN_STRING) {
      ...
      if (!key_stored) {
        ...
        strncpy(kv_pairs[nth_pair].key, &json[start_index],
                (JSON_KEY_LEN < str_len ? JSON_KEY_LEN : str_len));

        // Add string termination character
        kv_pairs[nth_pair]
            .key[(JSON_KEY_LEN < str_len ? JSON_KEY_LEN : str_len)] = '\0';
        ...
        strncpy(kv_pairs[nth_pair].val, &json[start_index],
                (JSON_VAL_LEN < str_len ? JSON_VAL_LEN : str_len));

        // Add string termination character
        kv_pairs[nth_pair]
            .val[(JSON_KEY_LEN < str_len ? JSON_KEY_LEN : str_len)] = '\0';
...
int json_stringify(json_t *kv_pairs, size_t kv_pairs_len, char *json_str,
  ...
  for (int x = 0; x < kv_pairs_len && json_str_x < json_str_len; x++) {
    ...
    int next_kv_str_len =
        snprintf(next_kv_str, json_str_len - json_str_x, "\"%s\":\"%s\",",
                 kv_pairs[x].key, kv_pairs[x].val);

...
int ipc_qsend(const char *dest, char *msg, size_t msg_len)
{
    ...
    for (int x = 0; x < msg_len && x < MAX_MSG_LEN; x++)
        qsend_msg[x] = msg[x];
...
int ipc_refresh()
{
    ...
    {
        // Read data
        if (read(self.conn.rx, qrecv_buf,
                 (qrecv_buf_len > MAX_MSG_LEN ? MAX_MSG_LEN : qrecv_buf_len)) <=
            0)
        { // read() failed
            // Check if read() should have blocked
            if (errno == EWOULDBLOCK || errno == EAGAIN)
        ...
        // int bytes_read = -1;
        // if((bytes_read = ipc_recv(qrecv_src, qrecv_buf, qrecv_buf_len)) <= 0)
        // // ipc_recv() failed
    ...
    { // qsend is good to go
        // Send data
        if (ipc_send(qsend_dest, qsend_msg, qsend_msg_len))
        { // send() failed
            fprintf(stderr, "ipc_send() failed\n");
            return -1;
        }

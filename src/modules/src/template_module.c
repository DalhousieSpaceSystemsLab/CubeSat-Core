...
int template_server_start(void* data) {
   ...
   *  3. May suddenly terminate the thread used to initialize your server (see
   * stop())
   *
   * Things NOT to do:
   *  1. Do not throw errors, they won't be caught.
   *  2. Do not quit the start method using 'return'. Instead use
   * pthread_exit(ptr).
...
int template_server_stop(void* data) {
   ...
   *
   * Specifically, the subsystem dock will stop or restart your server the
   * following way:
   *  1. Terminates the thread which initially ran the start() method.
   *  2. Runs the stop() method once and NOT as a separate thread.
   *  3. Depending on the desired outcome, may immediately initialize start()
   * again or do nothing.
   *
   * Things NOT to do:
   *  1. Do not throw errors, they won't be caught.

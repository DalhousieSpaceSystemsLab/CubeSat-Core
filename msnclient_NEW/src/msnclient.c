...
// modify sorting function to return the index or value of the mission that
// starts the soonest
    ...
    if (file != NULL)
    {
        ...
        for (int lineNumber = 0; lineNumber < 6; lineNumber++)
        {
            ...
            while (fgets(line, line_len, file) != NULL)
            {
                ...
                if (count ==
                    lineNumber) // if the current line is the specified one
                {
                    // parse the first 24 characters in the line (the date and
                    // time) and store in msnTime
                    for (int i = 0; i < 24; i++)
                    {
                        msnTime[i] = line[i];
                    }
                    ...
                    if (is_current_min(msnTime) == 0)
                    {
                        ...
                        int k = 0;
                        // parse the remainder of the line (the mission) and
                        // store in mission
